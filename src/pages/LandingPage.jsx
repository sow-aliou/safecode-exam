...
export default function LandingPage() {
  ...
  return (
    <div className="gradient-bg landing-page" style={{ position: 'relative' }}>
      ...
      <div style={{ position: 'absolute', top: 24, right: 24, display: 'flex', gap: 4, background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: 4, border: '1px solid var(--border)' }}>
        <button onClick={() => setLanguage('fr')} style={{
          background: lang === 'fr' ? 'var(--accent)' : 'transparent',
          color: '#fff', border: 'none', padding: '6px 12px', borderRadius: 7, cursor: 'pointer',
          fontSize: '0.8rem', fontWeight: 700, transition: 'background 0.2s'
        }}>🇫🇷 FR</button>
      ...
      {/* Feature badges */}
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center', animation: 'fadeUp 0.6s ease forwards' }}>
        {['🔒 Mode Kiosque', '💻 Code Editor', '📐 UML', '📊 Auto-correction', '🌐 Offline/Online'].map(f => (
          <span key={f} style={{
            background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.2)',
            borderRadius: 20, padding: '4px 14px', fontSize: '0.78rem', color: 'var(--accent-light)'
          }}>{f}</span>
        ))}
      </div>
